      ...
      <div class="bg-white rounded-2xl shadow-2xl p-8 sm:p-10 border border-white/50 form-animate">
        ...
        <div class="text-center mb-8">
          <h1 class="text-3xl font-bold text-gray-900 tracking-tight">
           <span class="text-cyan-600">Регистрация</span>
          </h1>
          <p class="text-gray-500 mt-2 text-sm">Создай аккаунт ученика</p>
        </div>
        ...
        <form @submit.prevent="handleRegister" class="space-y-5">
          ...
          <!-- Поле: Имя -->
          <div>
            <label for="name" class="block text-sm font-medium text-gray-700 mb-1">Как тебя зовут?</label>
            <input
              v-model="form.name"
              type="text"
              id="name"
        ...
        <div class="mt-6 text-center text-sm text-gray-500">
          Уже есть аккаунт? 
          <nuxt-link to="/users/login" class="text-cyan-600 hover:text-cyan-700 font-semibold hover:underline transition-colors">Войти</nuxt-link>
        </div>
      </div>
    </div>
